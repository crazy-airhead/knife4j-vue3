...
SwaggerBootstrapUi.prototype.analysisSpringDocOpenApiGroupSuccess = function (data) {
  ...
  globalsStore.setServiceOptions(serviceOptions)
  ...
  if (serviceOptions.length > 0) {
    ...
    globalsStore.setDefaultService(serviceOptions[0].value)
  }
}
  ...
  if (KUtils.checkUndefined(urlParams)) {
    if (urlParams.hasOwnProperty('groupName')) {
      ...
      if (KUtils.checkUndefined(gpName) && gpName != '') {
        ...
        globalsStore.setDefaultService(selectInstance.id);
...
SwaggerBootstrapUi.prototype.analysisApi = function (instance) {
  ...
  try {
    ...
    if (!that.currentInstance.load) {
      ...
      const globalsStore = useGlobalsStore()
      globalsStore.setSwaggerInstance(this.currentInstance)
    }
  } catch (err) {
    that.error(err);
    if (window.console) {
...
SwaggerBootstrapUi.prototype.dispatchSettings = function () {
  ...
  globalsStore.setResponseCode(this.settings.enableResponseCode)
  globalsStore.setSettings(this.settings)
}
          ...
          if (KUtils.arrNotEmpty(notEmptyConsumes)) {
            ...
            if (xmlRequest) {
              ...
              if (KUtils.strNotBlank(tmpJsonValue)) {
                ...
                swpinfo.requestValue = tmpJsonObject;
        ...
        if (KUtils.arrNotEmpty(swpinfo.consumes)) {
          ...
          if (KUtils.arrNotEmpty(notEmptyConsumes)) {
            ...
            if (xmlRequest) {
              // 是Xml请求
              if (KUtils.strNotBlank(tmpJsonValue)) {
                var c = KUtils.json5parse(tmpJsonValue);
                // var builder = new xml2js.Builder({
                //   rootName: tmpRootXmlName
                // });
                // var obj = builder.buildObject(tmpJsonObject);
...
SwaggerBootstrapUi.prototype.assembleParameterOAS3 = function (m, swpinfo, requireArray) {
  ...
  // https://gitee.com/xiaoym/knife4j/issues/I5GZ2Q
  if (m.hasOwnProperty('example')) {
    minfo.example = KUtils.propValue('example', m, '');
    minfo.txtValue = KUtils.propValue('example', m, '');
  }
  ...
  if (m.hasOwnProperty('schema')) {
    ...
    if (schemaType == 'array') {
      ...
      if (schemaObject.hasOwnProperty('default')) {
        minfo.example = KUtils.propValue('default', schemaObject, null);
        minfo.txtValue = KUtils.propValue('default', schemaObject, null);
      }
    } else {
      if (schemaObject.hasOwnProperty('$ref')) {
        var ref = m['schema']['$ref'];
